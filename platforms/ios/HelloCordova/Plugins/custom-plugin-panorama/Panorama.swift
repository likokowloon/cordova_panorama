...
@objc(Panorama) class Panorama : UIViewController, MonitorDelegate, UIGestureRecognizerDelegate {
    ...
    override func loadView() {
        ...
        self.tapRecognizer = UITapGestureRecognizer(target: self, action: #selector(Panorama.userTapped))
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        self.restart(nil)
        NotificationCenter.default.addObserver(self, selector: #selector(Panorama.willEnterForeground), name: .UIApplicationWillEnterForeground, object: nil)
    ...
    func userTapped(gs: UIGestureRecognizer){
        if(numShots==1){
            self.restart(gs)
        }else if(numShots!>1){
            self.stop(gs)
